...
for f in `ls *AOD*.root`; do
    ...
    try:
        if mode=='lhe':
            ...
            os.system('cp runOffGridpack%sPileupCondorLxplusSmallLifetimes.sh %s/submit' % (year,workpath))
            os.system('cp DIGIRAWHLT_template_%s.py %s/submit' % (year,workpath))
            with open('%s/submit/runOffGridpack%sPileupCondorLxplusSmallLifetimes.sh' % (workpath,year), 'a') as f:
                f.write(stageOutPiece)
    except:
...
exit 0'''
    with open(workpath + '/exec.sh', 'w') as f:
        ...
        if mode == 'lhe':
            ...
            f.write(execF % (('runOffGridpack%sPileupCondorLxplusSmallLifetimes' % year), infile+' '+ctau))
