...
options.register('inputString', '', VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "String in Root File Name")
options.register('lifetime', 0, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.int, "cTau of Data")	
options.register('lifetimeUnits', '', VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "lifetime units")
...
if options.test:
    ...
    if 'cmslpc' in platform.node():
        ...
        sampledir = 'root://eosuser.cern.ch///eos/user/a/alalbert/iDM/Samples/'
	inputstring = options.inputString
	inputFiles = []
        ctau = options.lifetime # mm
	os.chdir("/eos/user/a/alalbert/iDM/Samples")
	os.system("INPUTSTRING="+inputstring)
	listOutput = subprocess.check_output("ls")
	longList = re.split('\n',listOutput)
	#print(longList)
	secondInput = "ctau-"+str(options.lifetime)+"_year"
	if options.lifetimeUnits == 'microns' or options.lifetimeUnits == 'um' or options.lifetimeUnits == 'micrometers':
	    secondInput = 'ctauSmall-'+str(options.lifetime)+"_year"
	    ctau = ctau/1000.
	print(secondInput)
	for x in longList:
   	    if inputstring in x and secondInput in x:
        	inputFiles.append(x)
	print(inputFiles)
	...
	fnames = inputFiles
        ...
        #options.maxEvents = int(sys.argv[3])
        #options.outputFile = 'test1234_ctau%smm.root' % ctau
	#options.outputFile = options.inputString+'.root'
	#options.outputFile = 'testforerror.root'
else:
    options.maxEvents = -1 
...
process.load('iDMAnalysis.skimmer.myMETFilters_cff')
...
if options.year == 2016:
    ...
    from iDMAnalysis.skimmer.myBTagging_cff import addBTaggingAK4CHS
...
from iDMAnalysis.skimmer.muonsFromdSA_cff import * 
