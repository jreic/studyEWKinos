...
## produced: {1, 10, 100, 1000} mm per seed. ## changed to 1, 10, 100 microns
##
...
for ctau_mm in 1 10 100
#for ctau_mm in 10
do
    cp "${BASEDIR}/conf/${HADRONIZER}_ctauSmall-${ctau_mm}.py" Configuration/GenProduction/python/
    eval `scram runtime -sh`
    scram b -j 4
    echo "1.) Generating GEN-SIM for lifetime ${ctau_mm}"
    genfragment=${namebase}_GENSIM_cfg_ctauSmall-${ctau_mm}.py
    cmsDriver.py Configuration/GenProduction/python/${HADRONIZER}_ctauSmall-${ctau_mm}.py \
        --filein file:${LHEDIR}/${namebase}.lhe \
        --fileout file:${namebase}_GENSIM_ctauSmall-${ctau_mm}.root \
    ...
    sed -i "s/file:placeholder_in.root/file:${namebase}_GENSIM_ctauSmall-${ctau_mm}.root/g" DIGIRAWHLT_template_2018.py
    sed -i "s/file:placeholder_out.root/file:${namebase}_DIGIRAWHLT_ctauSmall-${ctau_mm}.root/g" DIGIRAWHLT_template_2018.py
    sed -i "s/input = cms.untracked.int32(10)/input = cms.untracked.int32(${nevent})/g" DIGIRAWHLT_template_2018.py
    mv DIGIRAWHLT_template_2018.py ${namebase}_DIGIRAWHLT_cfg_ctauSmall-${ctau_mm}.py

    #echo "2.) Generating DIGI-RAW-HLT for lifetime ${ctau_mm}"
    #cmsDriver.py step1 \
    #    --filein file:${namebase}_GENSIM_ctauSmall-${ctau_mm}.root \
    #    --fileout file:${namebase}_DIGIRAWHLT_ctauSmall-${ctau_mm}.root \
    ...
    #    --python_filename ${namebase}_DIGIRAWHLT_cfg_ctauSmall-${ctau_mm}.py \
    #    --customise Configuration/DataProcessing/Utils.addMonitoring \
    #    --no_exec || exit $?;
    cmsRun -p ${namebase}_DIGIRAWHLT_cfg_ctauSmall-${ctau_mm}.py

    echo "3.) Generating AOD for lifetime ${ctau_mm}"
    cmsDriver.py step2 \
        --filein file:${namebase}_DIGIRAWHLT_ctauSmall-${ctau_mm}.root \
        --fileout file:${namebase}_AOD_ctauSmall-${ctau_mm}_year-2018.root \
        ...
        --procModifiers premix_stage2 \
        --nThreads 1 --era Run2_2018 --python_filename ${namebase}_AOD_cfg_ctauSmall-${ctau_mm}.py --no_exec \
        --customise Configuration/DataProcessing/Utils.addMonitoring -n ${nevent} || exit $?;
    cmsRun -p ${namebase}_AOD_cfg_ctauSmall-${ctau_mm}.py
